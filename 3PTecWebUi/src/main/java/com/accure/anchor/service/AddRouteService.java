 ...
 * @author Abhishek
 */
public class AddRouteService extends HttpServlet {

    Logger logger = Logger.getLogger(AddRouteService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("AddRouteService", DmsConstants.ADD, "started"));
                ...
                if (authorized) {
                    logger.info(Common.getLogMsg("AddRouteService", DmsConstants.ADD, DmsConstants.AUTHORIZED));
                    String routeJson = request.getParameter("routeJson");
                  //add route
                    boolean status = new DmsRouteManager().createRoute(routeJson);
                    if (status) {
                        ...
                        logger.info(Common.getLogMsg("AddRouteService", DmsConstants.ADD, DmsConstants.SUCCESS));
                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("AddRouteService", DmsConstants.ADD, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("AddRouteService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("AddRouteService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("AddRouteService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
