 ...
 * @author user
 */
public class CheckUserAvailabilityService extends HttpServlet {

    Logger logger = Logger.getLogger(CheckUserAvailabilityService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("CheckUserAvailabilityService", DmsConstants.CHECK, "started"));
...
//                boolean authorized = new UserManager().checkUserPrivilege(currentUser, "CreateUsers");
                if (true) {
                    logger.info(Common.getLogMsg("CheckUserAvailabilityService", DmsConstants.CHECK, DmsConstants.AUTHORIZED));
                    String username = request.getParameter("loginId");
//                    String id = request.getParameter("id");
                    // Application level logging
//                    logger.info(Common.getLogMsg((String) currentUser.getOrgRole().get(0).getOrg().getId(), ((Map<String, String>) (currentUser.getId())).get("$oid"), new Gson().toJson(data), DmsConstants.CHECK, request.getRemoteAddr()));
                    boolean status = new DmsUserManager().checkUserAvailability(username);
                    ...
                    if (status) {
                        ...
                        logger.info(Common.getLogMsg("CheckUserAvailabilityService", DmsConstants.CHECK, DmsConstants.SUCCESS));
                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("CheckUserAvailabilityService", DmsConstants.CHECK, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("CheckUserAvailabilityService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("CheckUserAvailabilityService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("CheckUserAvailabilityService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
