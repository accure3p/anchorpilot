...
public class CreateInvoiceService extends HttpServlet {
    ...
    Logger logger = Logger.getLogger(CreateInvoiceService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("CreateInvoiceService", DmsConstants.CREATE, "started"));
                ...
                if (authorized) {
                    ...
                    logger.info(Common.getLogMsg("CreateInvoiceService", DmsConstants.CREATE, DmsConstants.AUTHORIZED));
...
//                    logger.info(Common.getLogMsg((String) currentUser.getId(), ((Map<String, String>) (currentUser.getId())).get("$oid"), new Gson().toJson(data), DmsConstants.VIEW, request.getRemoteAddr()));
                    String strResultJson = new DmsOrderManager().createInvoice(orderId, invoiceJson, invoiceLineItemJson,strUserFullName);

                    if (strResultJson != null) {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                         out.write(strResultJson);
                        logger.info(Common.getLogMsg("CreateInvoiceService", DmsConstants.CHECK, DmsConstants.SUCCESS));
                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("CreateInvoiceService", DmsConstants.CHECK, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("CreateInvoiceService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("CreateInvoiceService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("CreateInvoiceService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
