    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("createUserService", DmsConstants.CREATE, "started"));
                ...
                if (authorized) {
                    logger.info(Common.getLogMsg("createUserService", DmsConstants.CREATE, DmsConstants.AUTHORIZED));
                    String userDetails = request.getParameter("userDetails");
                    String userAddress = request.getParameter("userAddress");
                  //for create user
                    boolean status = new DmsUserManager().createUser(userDetails, userAddress);
                    if (status) {
                        ...
                        logger.info(Common.getLogMsg("createUserService", DmsConstants.CREATE, DmsConstants.SUCCESS));
                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("createUserService", DmsConstants.CREATE, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("createUserService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("createUserService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("createUserService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
