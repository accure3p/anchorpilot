 ...
 * @author accure
 */
public class FetchChildOrganizationsService extends HttpServlet {

    Logger logger = Logger.getLogger(FetchChildOrganizationsService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("FetchChildOrganizationsService", DmsConstants.VIEW, "started"));
                ...
                if (authorized) {
                    ...
                    logger.info(Common.getLogMsg("FetchChildOrganizationsService", DmsConstants.VIEW, DmsConstants.AUTHORIZED));
...
//                    logger.info(Common.getLogMsg((String) currentUser.getId(), ((Map<String, String>) (currentUser.getId())).get("$oid"), new Gson().toJson(data), DmsConstants.VIEW, request.getRemoteAddr()));
                    String resultJson = new DmsUserManager().fetchChildOrganizations(orgId);
                    if (resultJson != null) {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                        out.write(resultJson);
                        logger.info(Common.getLogMsg("FetchChildOrganizationsService", DmsConstants.VIEW, DmsConstants.SUCCESS));

                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("FetchChildOrganizationsService", DmsConstants.VIEW, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("FetchChildOrganizationsService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("FetchChildOrganizationsService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("FetchChildOrganizationsService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
