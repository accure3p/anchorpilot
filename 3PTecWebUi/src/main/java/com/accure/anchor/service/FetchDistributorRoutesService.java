 ...
 * @author rajeev
 */
public class FetchDistributorRoutesService extends HttpServlet {
    Logger logger = Logger.getLogger(FetchDistributorRoutesService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("FetchDistributorRoutesService", DmsConstants.VIEW, "started"));
                ...
                if (authorized) {
                    logger.info(Common.getLogMsg("FetchDistributorRoutesService", DmsConstants.VIEW, DmsConstants.AUTHORIZED));
                    String strDistributorId = request.getParameter("distributorId");
                    String strzoneId = request.getParameter("zoneId");
                    
                    String resultJson = new DmsRetailerManager().getDistributorRoutes(strDistributorId,strzoneId);
                    if (resultJson != null) {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                        out.write(resultJson);
                        logger.info(Common.getLogMsg("FetchDistributorRoutesService", DmsConstants.VIEW, DmsConstants.SUCCESS));
                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("FetchDistributorRoutesService", DmsConstants.VIEW, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("FetchDistributorRoutesService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("FetchDistributorRoutesService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("FetchDistributorRoutesService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
