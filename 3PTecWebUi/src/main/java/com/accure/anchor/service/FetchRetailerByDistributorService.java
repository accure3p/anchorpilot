 ...
 * @author user
 */
public class FetchRetailerByDistributorService extends HttpServlet {

    Logger logger = Logger.getLogger(FetchRetailerByDistributorService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("FetchRetailerByDistributorService", DmsConstants.VIEW, "started"));
                ...
                if (authorized) {
                    ...
                    logger.info(Common.getLogMsg("FetchRetailerByDistributorService", DmsConstants.VIEW, DmsConstants.AUTHORIZED));
...
//                    logger.info(Common.getLogMsg((String) currentUser.getId(), ((Map<String, String>) (currentUser.getId())).get("$oid"), new Gson().toJson(data), DmsConstants.VIEW, request.getRemoteAddr()));
                    String resultJson = new DmsRetailerManager().viewAllRetailers(orgId);
                    request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                    out.write(resultJson);
                    logger.info(Common.getLogMsg("FetchRetailerByDistributorService", DmsConstants.VIEW, DmsConstants.SUCCESS));
                } else {
                    ...
                    logger.info(Common.getLogMsg("FetchRetailerByDistributorService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("FetchRetailerByDistributorService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("FetchRetailerByDistributorService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
