 ...
 * @author user
 */
public class FetchRetailerNameService extends HttpServlet {

    Logger logger = Logger.getLogger(FetchRetailerNameService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("FetchRetailerNameService", DmsConstants.VIEW, "started"));
                ...
                if (authorized) {
                    ...
                    logger.info(Common.getLogMsg("FetchRetailerNameService", DmsConstants.VIEW, DmsConstants.AUTHORIZED));
                    String zonId = request.getParameter("zonId");
                    String data = "zonId-" + zonId;
...
//                    String resultJson = new DmsRouteManager().fetchOrgNameByZone(zonId);

                    String resultJson = new DmsRouteManager().fetchRetailersByZoneId(zonId);

                    request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                    out.write(resultJson);
                    logger.info(Common.getLogMsg("FetchRetailerNameService", DmsConstants.VIEW, DmsConstants.SUCCESS));
                } else {
                    ...
                    logger.info(Common.getLogMsg("FetchRetailerNameService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("FetchRetailerNameService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("FetchRetailerNameService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
