 ...
 * @author Abhishek
 */
public class FetchSalesExecutiveService extends HttpServlet {

    Logger logger = Logger.getLogger(FetchSalesExecutiveService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("FetchSalesExecutiveService", DmsConstants.VIEW, "started"));
                ...
                if (authorized) {
                    ...
                    logger.info(Common.getLogMsg("FetchSalesExecutiveService", DmsConstants.VIEW, DmsConstants.AUTHORIZED));
...
//                    logger.info(Common.getLogMsg((String) currentUser.getId(), ((Map<String, String>) (currentUser.getId())).get("$oid"), new Gson().toJson(data), DmsConstants.VIEW, request.getRemoteAddr()));
                    String resultJson = new DmsRouteManager().viewSalesExecutive(orgId);
                    request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                    out.write(resultJson);
                    logger.info(Common.getLogMsg("FetchSalesExecutiveService", DmsConstants.VIEW, DmsConstants.SUCCESS));
                } else {
                    ...
                    logger.info(Common.getLogMsg("FetchSalesExecutiveService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("FetchSalesExecutiveService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("FetchSalesExecutiveService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
