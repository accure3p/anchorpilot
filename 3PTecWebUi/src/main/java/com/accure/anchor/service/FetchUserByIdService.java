 ...
 * @author accure
 */
public class FetchUserByIdService extends HttpServlet {

   Logger logger = Logger.getLogger(FetchUserByIdService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("FetchUserByIdService", DmsConstants.VIEW, "started"));
                ...
                if (authorized) {
                    ...
                    logger.info(Common.getLogMsg("FetchUserByIdService", DmsConstants.VIEW, DmsConstants.AUTHORIZED));
                    String userId = request.getParameter("userId");
                    String data = "orgId-" + userId;
...
//                    logger.info(Common.getLogMsg((String) currentUser.getId(), ((Map<String, String>) (currentUser.getId())).get("$oid"), new Gson().toJson(data), DmsConstants.VIEW, request.getRemoteAddr()));
                    String resultJson = new DmsUserManager().viewUserDetailById(userId);
                    if (resultJson != null) {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                        out.write(resultJson);
                        logger.info(Common.getLogMsg("FetchUserByIdService", DmsConstants.VIEW, DmsConstants.SUCCESS));

                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("FetchUserByIdService", DmsConstants.VIEW, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("FetchUserByIdService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("FetchUserByIdService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("FetchUserByIdService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
