 ...
 * @author user
 */
public class FetchUserByRolesService extends HttpServlet {

    Logger logger = Logger.getLogger(FetchUserByRolesService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("FetchUserByRolesService", DmsConstants.VIEW, "started"));
                ...
                if (authorized) {
                    ...
                    logger.info(Common.getLogMsg("FetchUserByRolesService", DmsConstants.VIEW, DmsConstants.AUTHORIZED));
                    String orgId = request.getParameter("orgId");
                    String role = request.getParameter("role");
                    String data = "userId-" + orgId;
...
//                    logger.info(Common.getLogMsg((String) currentUser.getId(), ((Map<String, String>) (currentUser.getId())).get("$oid"), new Gson().toJson(data), DmsConstants.VIEW, request.getRemoteAddr()));
                    String resultJson = new DmsUserManager().fetchUserByRoles(orgId,role);
                    request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                    out.write(resultJson);
                    logger.info(Common.getLogMsg("FetchUserByRolesService", DmsConstants.VIEW, DmsConstants.SUCCESS));
                } else {
                    ...
                    logger.info(Common.getLogMsg("FetchUserByRolesService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("FetchUserByRolesService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("FetchUserByRolesService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
