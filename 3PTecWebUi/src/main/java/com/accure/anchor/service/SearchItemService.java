 ...
 * @author user
 */
public class SearchItemService extends HttpServlet {

    Logger logger = Logger.getLogger(SearchItemService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("SearchItemService", DmsConstants.VIEW, "started"));
                ...
                if (authorized) {
                    ...
                    logger.info(Common.getLogMsg("SearchItemService", DmsConstants.VIEW, DmsConstants.AUTHORIZED));
                    ...
                    String data = "orgId-" + parentorgid + "searchKey-" + searchKey + "sindex-" + sindex + "limit-" + limit;
...
//                    logger.info(Common.getLogMsg((String) currentUser.getId(), ((Map<String, String>) (currentUser.getId())).get("$oid"), new Gson().toJson(data), DmsConstants.VIEW, request.getRemoteAddr()));
                    String resultJson = new DmsOrderManager().searchItem(searchKey, sindex, limit, parentorgid);
                    request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_SUCCESS);
                    out.write(resultJson);
                    logger.info(Common.getLogMsg("SearchItemService", DmsConstants.VIEW, DmsConstants.SUCCESS));
                } else {
                    ...
                    logger.info(Common.getLogMsg("SearchItemService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("SearchItemService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("SearchItemService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
