 ...
 * @author user
 */
public class UpdateRetailerService extends HttpServlet {

    Logger logger = Logger.getLogger(UpdateRetailerService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                logger.info(Common.getLogMsg("UpdateRetailerService", DmsConstants.UPDATE, "started"));
                ...
                if (authorized) {
                    logger.info(Common.getLogMsg("UpdateRetailerService", DmsConstants.UPDATE, DmsConstants.AUTHORIZED));
...
//for create u  ser
                    boolean status = new DmsRetailerManager().updateRetailer(retailerId, addressId, retailerJson, addressJson);
                    if (status) {
                        ...
                        logger.info(Common.getLogMsg("UpdateRetailerService", DmsConstants.UPDATE, DmsConstants.SUCCESS));
                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("UpdateRetailerService", DmsConstants.UPDATE, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("UpdateRetailerService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("UpdateRetailerService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("UpdateRetailerService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
