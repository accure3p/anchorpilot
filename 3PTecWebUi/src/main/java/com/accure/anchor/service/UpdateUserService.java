 ...
 * @author Abhishek
 */
public class UpdateUserService extends HttpServlet {

    Logger logger = Logger.getLogger(UpdateUserService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (AnchorSessionManager.checkUserSession(session)) {
                ...
                logger.info(Common.getLogMsg("UpdateUserService", DmsConstants.UPDATE, "started"));
                ...
                if (authorized) {
                    logger.info(Common.getLogMsg("UpdateUserService", DmsConstants.UPDATE, DmsConstants.AUTHORIZED));
                    String userJson = request.getParameter("userDetails");
                    String addressJson = request.getParameter("userAdress");
                    String userId = request.getParameter("userId");
//for update user
                    boolean status = new DmsUserManager().updateUser(userJson, addressJson, userId);
                    if (status) {
                        ...
                        logger.info(Common.getLogMsg("UpdateUserService", DmsConstants.UPDATE, DmsConstants.SUCCESS));
                    } else {
                        request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(DmsConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("UpdateUserService", DmsConstants.UPDATE, DmsConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("UpdateUserService", DmsConstants.FAIL, DmsConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                ...
                logger.info(Common.getLogMsg("UpdateUserService", DmsConstants.FAIL, DmsConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", DmsConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("UpdateUserService", DmsConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
